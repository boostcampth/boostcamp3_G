...
public class FeedFirebaseService implements FeedDataSource {
    ...
    @Override
    @NonNull
    public Single<PagedListResponse<LegacyFeed>> addMainFeedList(
            int pageIndex,
            int pageUnit) {
        // TODO : 결과 테스트 필요 DONE
        ...
        return Single.create(emitter -> FirebaseFirestore.getInstance().collection(COLLECTION_FEED)
                ...
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        ...
                        List<LegacyFeed> feedList = new ArrayList<>();
                        for (QueryDocumentSnapshot document : Objects.requireNonNull(task.getResult())) {
                            LegacyFeed feed = document.toObject(LegacyFeed.class);
    ...
    private void uploadImageStorage(@NonNull LegacyFeed feed, Uri uri, String position) {
